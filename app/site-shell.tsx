...
export function SiteShell({children,active}:{children:ReactNode;active:string}){return <>
 <header className="site-header"><div className="container nav-wrap">
  ...
  <details className="mobile-menu"><summary aria-label="Ouvrir le menu"><span className="menu-bars"><i/><i/><i/></span></summary><nav><div className="menu-brand"><Image src="/veralya-logo-hq.png" alt="" width={44} height={44}/><strong>VERALYA</strong></div>{nav.map(([id,url,label])=><Link className={active===id?"active":""} href={url} key={id}>{label}</Link>)}<Link className="mobile-join" href="/contact">Rejoindre <Arrow/></Link></nav></details>
