...
export function SiteShell({children,active}:{children:ReactNode;active:string}){return <>
 <header className="site-header"><div className="container nav-wrap">
  <Link className="brand" href="/"><Image src="/veralya-logo.png" alt="Logo Veralya" width={52} height={52}/><strong>VERALYA</strong></Link>
  ...
  <details className="mobile-menu"><summary aria-label="Ouvrir le menu">☰</summary><nav>{nav.map(([id,url,label])=><Link href={url} key={id}>{label}</Link>)}</nav></details>
 </div></header>{children}<Footer/></>}

function Footer(){return <footer><div className="container footer-grid"><div><Link className="brand" href="/"><Image src="/veralya-logo.png" alt="Logo Veralya" width={48} height={48}/><strong>VERALYA</strong></Link><p>Elevate Performance System.</p></div><div className="footer-links"><a href="https://www.instagram.com/veralyaltp" target="_blank" rel="noreferrer">Instagram</a><a href="https://ma.linkedin.com/in/veralya-learn-transform-perform-7188a0407" target="_blank" rel="noreferrer">LinkedIn</a><Link href="/contact">Newsletter</Link><Link href="/a-propos">Mentions Légales</Link></div><p className="copyright">© 2026 VERALYA — Elevate Performance System. Tous droits réservés. Casablanca, Maroc.</p></div></footer>}
